 ...
 */
int hashmap_put(map_t in, const char* key, const any_t value){
    int index;
    hashmap_map* m;
    ...
    /* Set the data */
    m->data[index] = (hashmap_element) { key, 1, value };
    m->size++; 
 ...
 */
int hashmap_iterate(map_t in, const PFany f, any_t item) {
    int i;
...
int hashmap_remove(map_t in, const char* key){
    ...
    for(i = 0; i<MAX_CHAIN_LENGTH; i++){
        ...
        if (in_use == 1){
            if (strcmp(m->data[curr].key,key)==0){
                m->data[curr] = (hashmap_element) { NULL, 0, NULL };
                m->size--;
                return MAP_OK;
            }
        }
...
int hashmap_length(const map_t in){
    hashmap_map* m = (hashmap_map *) in;
